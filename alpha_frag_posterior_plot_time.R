...
sstd_study_posterior_ts <- ggplot() +
  geom_rect(data = Sstd_posterior %>% distinct(Sstd_lower_slope, Sstd_upper_slope),
            ...
            alpha = 0.75) +
  geom_point(data = Sstd_posterior %>% 
               group_by(time.since.fragmentation, sphere.fragment, sphere.matrix) %>% 
               summarise(median_slope = median(S_std + unique(Sstd_global_slope))),
             aes(y = median_slope, x = time.since.fragmentation,
                 group = interaction(time.since.fragmentation, sphere.fragment, sphere.matrix),
                 colour = interaction(sphere.fragment, sphere.matrix)),
             position = position_dodge(width = 0.25),
             size = 3) +
  # 75% CI
  geom_linerange(data = Sstd_posterior %>% 
...
ggsave('~/Dropbox/Frag Database (new)/analysis_apr19/figures/supplement_time.png',
       width = 350,
       height = 220, 
       units = 'mm')
