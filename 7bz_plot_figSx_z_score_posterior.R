...
source(paste0(path2wd, '5aaz_alpha_frag_posterior_z_score_wrangle.R'))
...
sstd_study_posterior_time <- ggplot() +
  ...
  geom_text(data = Sstd_posterior %>%
            ...
            aes(x=6, y=time.since.fragmentation, 
...
cowplot::plot_grid(top, bottom, rel_heights = c(0.05,1), nrow = 2) +
  cowplot::draw_label(expression(paste('Species richness z-score slope estimate')), y = 0.01)

ggsave('~/Dropbox/Frag Database (new)/Manuscript for Nature/revision1/figures/z_Sstd_posterior.png',
       width = 240,
       height = 220,
       units = 'mm')
# 
...
time <- ggplot() +
  ...
  geom_text(data = Spie_posterior %>%
              ...
              ungroup() %>% 
              distinct(time.since.fragmentation, n_study, .keep_all = T),
            aes(x=3, y=time.since.fragmentation, 
                label=paste('n[study] == ', n_study)),
            size=3.5,
            nudge_y = 0.1, parse = T) +
  theme_bw() +
...
cowplot::plot_grid(top, bottom, rel_heights = c(0.05,1), nrow = 2) +
  cowplot::draw_label(expression(paste('Evenness z-score slope estimate')), y = 0.01)

ggsave('~/Dropbox/Frag Database (new)/Manuscript for Nature/revision1/figures/z_S_PIE_posterior.png',
       width = 240,
       height = 220,
       units = 'mm')
