...
sstd_study_posterior_tt <- ggplot() +
  ...
  geom_point(data = Sstd_posterior %>% 
             ...
             aes(y = median_slope, x = frag_matrix,
                 group = interaction(frag_matrix, taxa),
                 colour = taxa),
...
sstd_study_posterior_tm <- ggplot() +
  geom_rect(data = Sstd_posterior %>% distinct(Sstd_lower_slope, Sstd_upper_slope),
            ...
            alpha = 0.75) +
  geom_point(data = Sstd_posterior %>% 
               group_by(frag_matrix, time.since.fragmentation) %>% 
               summarise(median_slope = median(S_std + unique(Sstd_global_slope)),
                         lower2.5 = quantile(S_std + unique(Sstd_global_slope), probs = 0.025),
                         lower25 = quantile(S_std + unique(Sstd_global_slope), probs = 0.25),
                         upper75 = quantile(S_std + unique(Sstd_global_slope), probs = 0.75),
                         upper97.5 = quantile(S_std + unique(Sstd_global_slope), probs = 0.975)),
             aes(y = median_slope, x = frag_matrix,
                 group = interaction(frag_matrix, time.since.fragmentation),
...
sstd_study_posterior_tb <- ggplot() +
  ...
  geom_point(data = Sstd_posterior %>% 
               ...
               summarise(median_slope = median(S_std + unique(Sstd_global_slope)),
                         ...
                         upper97.5 = quantile(S_std + unique(Sstd_global_slope), probs = 0.975)),
             aes(y = median_slope, x = frag_matrix,
                 group = interaction(frag_matrix, biome),
                 colour = biome),
             position = position_dodge(width = 0.25),
             size = 3) +
  # 75% CI
...
sstd_study_posterior_tc <- ggplot() +
  geom_rect(data = Sstd_posterior %>% distinct(Sstd_lower_slope, Sstd_upper_slope),
            ...
            alpha = 0.75) +
  geom_point(data = Sstd_posterior %>% 
               group_by(frag_matrix, continent8) %>% 
               summarise(median_slope = median(S_std + unique(Sstd_global_slope)),
                         lower2.5 = quantile(S_std + unique(Sstd_global_slope), probs = 0.025),
                         lower25 = quantile(S_std + unique(Sstd_global_slope), probs = 0.25),
                         upper75 = quantile(S_std + unique(Sstd_global_slope), probs = 0.75),
                         upper97.5 = quantile(S_std + unique(Sstd_global_slope), probs = 0.975)),
             aes(y = median_slope, x = frag_matrix,
                 group = interaction(frag_matrix, continent8),
                 colour = continent8),
...
sstd_study_posterior_tl <- ggplot() +
  ...
  geom_point(data = Sstd_posterior %>% 
               ...
               summarise(median_slope = median(S_std + unique(Sstd_global_slope)),
                         ...
                         upper97.5 = quantile(S_std + unique(Sstd_global_slope), probs = 0.975)),
             aes(y = median_slope, x = frag_matrix,
                 group = interaction(frag_matrix, climate),
                 colour = climate),
             position = position_dodge(width = 0.25),
             size = 3) +
  # 75% CI
...
sstd_study_posterior_ts <- ggplot() +
  geom_rect(data = Sstd_posterior %>% distinct(Sstd_lower_slope, Sstd_upper_slope),
            ...
            alpha = 0.75) +
  geom_point(data = Sstd_posterior %>% 
               group_by(frag_matrix, Matrix.category) %>% 
               summarise(median_slope = median(S_std + unique(Sstd_global_slope))),
             aes(y = median_slope, x = frag_matrix,
                 group = interaction(frag_matrix, Matrix.category),
...
bottom + 
  ...
  cowplot::draw_label('Fragment & matrix sphere', x = 0.005, angle = 90)
# ggsave('~/Dropbox/Frag Database (new)/analysis_apr19/figures/FigSx_frag_mat.png',
#        width = 350,
#        height = 220,
#        units = 'mm')
