...
three_grey_legend <- ggplot() +
  ...
  geom_density_ridges_gradient(data = Jtu_posterior,
                               aes(x = Jtu + unique(Jtu_global_slope),
...
Jtu_study_posterior_time <- ggplot() +
  # facet_grid(continent ~ ., scale = 'free') +
  geom_density_ridges_gradient(data = Jtu_posterior,
                               aes(x = Jtu + unique(Jtu_global_slope),
                                   y = time.since.fragmentation,
                               ...
                               scale = 0.9, alpha = 0.5,
                               linetype = 0) +
  # stat_density_ridges(data = Jtu_posterior,
...
Jtu_study_posterior_matrix <- ggplot() +
  geom_density_ridges_gradient(data = Jtu_posterior,
                               aes(x = Jtu + unique(Jtu_global_slope),
                                   y = Matrix.category,
                                   fill = stat(quantile)
...
Jtu_study_posterior_biome <- ggplot() +
  geom_density_ridges_gradient(data = Jtu_posterior,
                               aes(x = Jtu + unique(Jtu_global_slope),
                                   y = biome,
                                   fill = stat(quantile)
...
Jtu_study_posterior_taxa <- ggplot() +
  geom_density_ridges_gradient(data = Jtu_posterior,
                               aes(x = Jtu + unique(Jtu_global_slope),
                                   y = taxa,
                                   fill = stat(quantile)
...
Jtu_study_posterior_continent <- ggplot() +
  geom_density_ridges_gradient(data = Jtu_posterior,
                               aes(x = Jtu + unique(Jtu_global_slope),
                                   y = continent8,
                                   fill = stat(quantile)
...
Jtu_study_posterior_climate <- ggplot() +
  geom_density_ridges_gradient(data = Jtu_posterior,
                               aes(x = Jtu + unique(Jtu_global_slope),
                                   y = climate,
                                   fill = stat(quantile)
...
Jtu_study_posterior_sphere.frag <- ggplot() +
  geom_density_ridges_gradient(data = Jtu_posterior,
                               aes(x = Jtu + unique(Jtu_global_slope),
                                   y = frag_matrix,
                                   fill = stat(quantile)
...
bottom = cowplot::plot_grid(Jtu_study_posterior_continent,
                            Jtu_study_posterior_biome,
                            Jtu_study_posterior_sphere.frag,
                            Jtu_study_posterior_taxa,
                            Jtu_study_posterior_time, 
                            Jtu_study_posterior_matrix, 
                            nrow = 2)

cowplot::plot_grid(top, bottom, rel_heights = c(0.05,1), nrow = 2) +
  cowplot::draw_label('Study-level turnover', y = 0.02)

ggsave('~/Dropbox/Frag Database (new)/analysis_apr19/figures/turnover_posteriors.png',
       width = 290,
       height = 220,
       units = 'mm')
