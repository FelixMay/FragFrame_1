...
ES_frag_df <- ES_frag_df[,c("Case.ID","Study.ID", "n.fragment", "sample.design",
                            sapply(BDmetrics, function(x) paste(c("ES."),x,sep="")))]

ES_frag_group_df <- ES_frag_group_df[,c("Case.ID","Study.ID", "n.fragment", "sample.design",
                            sapply(BDmetrics, function(x) paste(c("ES."),x,sep="")))]

ES_df <- subset(ES_df, n.fragment>3)
ES_df <- ES_df[,c("Case.ID","Study.ID", "n.fragment", "sample.design",
