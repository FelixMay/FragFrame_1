...
three_grey_legend <- ggplot() +
  ...
  geom_density_ridges_gradient(data = Rne_posterior,
                               aes(x = Rne + unique(Rne_global_slope),
...
Rne_study_posterior_time <- ggplot() +
  # facet_grid(continent ~ ., scale = 'free') +
  geom_density_ridges_gradient(data = Rne_posterior,
                               aes(x = Rne + unique(Rne_global_slope),
                                   y = time.since.fragmentation,
                               ...
                               scale = 0.9, alpha = 0.5,
                               linetype = 0) +
  # stat_density_ridges(data = Rne_posterior,
  ...
  #                     calc_ecdf = T,
  #                     scale = 0.9, alpha = 0.5) +
  geom_rect(data = Rne_posterior %>% distinct(Rne_lower_slope, Rne_upper_slope),
            aes(xmin = Rne_lower_slope, xmax = Rne_upper_slope), ymin = -Inf, ymax = Inf,
            alpha = 0.6) +
  geom_point(data = Rne_posterior,
             aes(x = Rne + unique(Rne_global_slope), 
                 y = time.since.fragmentation),
             stat = ggstance:::StatSummaryh,
             fun.x = median,
             size = 2.5, shape = 18) +
  geom_vline(data = Rne_posterior,
             aes(xintercept = Rne_global_slope)) +
  geom_vline(xintercept = 0, lty = 2) +
  geom_text(data = Rne_posterior %>%
            ...
            aes(x=0.75, y=time.since.fragmentation, 
...
Rne_study_posterior_matrix <- ggplot() +
  geom_density_ridges_gradient(data = Rne_posterior,
                               aes(x = Rne + unique(Rne_global_slope),
                                   y = Matrix.category,
                                   fill = stat(quantile)
...
Rne_study_posterior_biome <- ggplot() +
  geom_density_ridges_gradient(data = Rne_posterior,
                               aes(x = Rne + unique(Rne_global_slope),
                                   y = biome,
                                   fill = stat(quantile)
...
Rne_study_posterior_taxa <- ggplot() +
  geom_density_ridges_gradient(data = Rne_posterior,
                               aes(x = Rne + unique(Rne_global_slope),
                                   y = taxa,
                                   fill = stat(quantile)
...
Rne_study_posterior_continent <- ggplot() +
  geom_density_ridges_gradient(data = Rne_posterior,
                               aes(x = Rne + unique(Rne_global_slope),
                                   y = continent8,
                                   fill = stat(quantile)
...
Rne_study_posterior_climate <- ggplot() +
  geom_density_ridges_gradient(data = Rne_posterior,
                               aes(x = Rne + unique(Rne_global_slope),
                                   y = climate,
                                   fill = stat(quantile)
...
Rne_study_posterior_sphere.frag <- ggplot() +
  geom_density_ridges_gradient(data = Rne_posterior,
                               aes(x = Rne + unique(Rne_global_slope),
                                   y = frag_matrix,
                                   fill = stat(quantile)
...
bottom = cowplot::plot_grid(Rne_study_posterior_continent,
                            Rne_study_posterior_biome,
                            Rne_study_posterior_sphere.frag,
                            Rne_study_posterior_taxa,
                            Rne_study_posterior_time, 
                            Rne_study_posterior_matrix, 
                            nrow = 2)

cowplot::plot_grid(top, bottom, rel_heights = c(0.05,1), nrow = 2) +
  cowplot::draw_label('Study-level abundance gradient change', y = 0.02)

ggsave('~/Dropbox/Frag Database (new)/analysis_apr19/figures/abund_grad_posteriors.png',
       width = 290,
       height = 220,
       units = 'mm')
