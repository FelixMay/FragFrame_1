...
for(i in 1:length(ES_df$Study.ID)){
   ...
   for(j in names(div_df)[-(1:3)]){
      ...
      r <- ifelse(ES_df[i,"n.fragment"] < 90,
                  2*sin(pi*cor(sub.df$entity.size.rank,sub.df[,j],method="spearman")/6),
                  cor(sub.df$entity.size.rank,sub.df[,j],method="spearman")) ### transform rank-correlation into Pearson-correlation, cf. Box 13.3. p 201 in Koricheva et al 2013
